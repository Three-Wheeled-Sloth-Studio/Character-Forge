...
function parseBoostPlan(value:string): Dnd5eAbilityIncreasePlan { const plan:Partial<Record<Dnd5eAbilityId,number>>={}; for(const part of value.split(",")){const [id,amount]=part.split(":"); if(!id||!amount)continue; plan[id as Dnd5eAbilityId]=Number(amount);} return plan; }
