...
function validateAbilityState(payload: JsonObject, issues: RulesValidationIssue[]): void {
  ...
  for (const abilityId of DND5E_ABILITY_IDS) {
    ...
    if (finalScore !== baseScore + increase) pushError(issues, "dnd5e.abilities.final-mismatch", `Final ${abilityId} must equal base score plus background increase.`, `abilities.final.${abilityId}`);
    if (!Number.isInteger(finalScore) || finalScore < 1 || finalScore > 20) pushError(issues, "dnd5e.abilities.range", `Final ${abilityId} must be an integer from 1 through 20 in this Level 1 slice.`, `abilities.final.${abilityId}`);
  }
}

function validateRangeBase(base: JsonObject, issues: RulesValidationIssue[], methodCode: string, methodLabel: string, minimum: number, maximum: number): void {
  for (const abilityId of DND5E_ABILITY_IDS) {
...
function validatePointCostBase(base: JsonObject, issues: RulesValidationIssue[]): void {
  ...
  if (allScoresValid && pointsSpent > DND5E_POINT_COST_BUDGET) pushError(issues, "dnd5e.point-cost.budget", `Point Cost ability scores spend ${pointsSpent} points, exceeding the ${DND5E_POINT_COST_BUDGET}-point budget.`, "abilities.base");
}
...
function validateAbilityMethodAndBackgroundBoosts(payload: JsonObject, issues: RulesValidationIssue[]): void {
  ...
  if (generationMethod === "standard-array") {
    const baseScores = DND5E_ABILITY_IDS.map((abilityId) => readNumber(base, abilityId)).filter((value): value is number => value !== undefined).sort((left, right) => right - left);
    const expectedStandardArray = [15, 14, 13, 12, 10, 8];
    if (baseScores.length !== expectedStandardArray.length || baseScores.some((score, index) => score !== expectedStandardArray[index])) pushError(issues, "dnd5e.standard-array.values", "Standard Array must use 15, 14, 13, 12, 10, and 8 exactly once.", "abilities.base");
  } else if (generationMethod === "manual") validateRangeBase(base, issues, "manual", "Manual", 3, 18);
  else if (generationMethod === "point-cost") validatePointCostBase(base, issues);
  else if (generationMethod === "random") validateRangeBase(base, issues, "random", "Random Generation", 3, 18);
  else pushError(issues, "dnd5e.slice.ability-method", "This slice supports Standard Array, Manual ability entry, Point Cost, and Random Generation.", "abilities.generationMethod");
  ...
  const increaseValues = Object.fromEntries(DND5E_ABILITY_IDS.map((abilityId) => [abilityId, readNumber(increases, abilityId) ?? 0])) as Record<Dnd5eAbilityId, number>;
  ...
  if (nonZeroIncreases.some((abilityId) => !allowedAbilities.has(abilityId)) || (!isTwoPlusOne && !isThreeOnes)) {
    pushError(issues, "dnd5e.background.ability-increases", `${background.label} ability increases must use +2/+1 on two different listed abilities, or +1 on all three listed abilities.`, "abilities.backgroundIncreases");
  }
}
...
function validateLevelOneIdentity(payload: JsonObject, issues: RulesValidationIssue[]): LevelOneSliceParts {
  ...
  if (identity && (readNumber(identity, "level") !== 1 || readNumber(identity, "experiencePoints") !== 0)) pushError(issues, "dnd5e.level-one.identity", "The current D&D generation slice requires a Level 1 character with 0 XP.", "identity");
  if (classState && readNumber(classState, "proficiencyBonus") !== 2) pushError(issues, "dnd5e.proficiency.level-one", "A Level 1 character must have a +2 Proficiency Bonus.", "class.proficiencyBonus");
...
function validateLegacyFirstSliceRules(payload: JsonObject, issues: RulesValidationIssue[]): void {
  ...
  if (readString(origin, "backgroundId") !== "soldier") pushError(issues, "dnd5e.slice.background", "The legacy first slice uses the Soldier background.", "origin.backgroundId");
  if (readString(origin, "speciesId") !== "human") pushError(issues, "dnd5e.slice.species", "The legacy first slice uses the Human species.", "origin.speciesId");
  if (readString(classState, "classId") !== "fighter" || readNumber(classState, "level") !== 1) pushError(issues, "dnd5e.slice.class", "The legacy first slice requires Fighter 1.", "class");
  ...
  if (constitution !== undefined && hpMaximum !== undefined) {
    ...
    if (hpMaximum !== expectedHp) pushError(issues, "dnd5e.fighter.level-one-hp", `A Level 1 Fighter with Constitution ${constitution} must have ${expectedHp} maximum Hit Points.`, "resources.hitPointsMaximum");
  }
}
...
function validateGuidedFirstSliceRules(payload: JsonObject, issues: RulesValidationIssue[]): void {
  ...
  if (!backgroundId || (backgroundId !== "criminal" && backgroundId !== "soldier")) {
    pushError(issues, "dnd5e.guided.background", "This retained dnd5e-character/0.2 validator supports Criminal and Soldier backgrounds.", "origin.backgroundId");
    return;
  }
  ...
  if (readString(origin, "backgroundOriginFeatId") !== backgroundExpected.featId) pushError(issues, "dnd5e.guided.background-feat", `${backgroundId} must retain its SRD Origin feat.`, "origin.backgroundOriginFeatId");
  if (!sameStringSet(readStringArray(origin, "backgroundSkillProficiencies"), backgroundExpected.skills)) pushError(issues, "dnd5e.guided.background-skills", `${backgroundId} must retain its two SRD skill proficiencies.`, "origin.backgroundSkillProficiencies");
  if (readString(origin, "toolProficiencyId") !== backgroundExpected.toolId) pushError(issues, "dnd5e.guided.background-tool", `${backgroundId} must retain its SRD tool proficiency.`, "origin.toolProficiencyId");
  const backgroundEquipmentChoice = readString(origin, "backgroundEquipmentChoice");
  if (backgroundEquipmentChoice !== "A" && backgroundEquipmentChoice !== "B:50-gp") pushError(issues, "dnd5e.guided.background-equipment", "Guided background equipment must use package A or the 50 GP option.", "origin.backgroundEquipmentChoice");
  if (readNumber(classState, "level") !== 1 || readNumber(classState, "hitDie") !== GUIDED_CLASS_HIT_DIE[classId]) pushError(issues, "dnd5e.guided.class-core", `Level 1 ${classId} must retain its SRD Hit Die and level.`, "class");
  if (readString(origin, "size") !== GUIDED_SPECIES_SIZE[speciesId] || readNumber(origin, "speedFeet") !== 30) pushError(issues, "dnd5e.guided.species-core", `${speciesId} size and Speed must match the retained SRD guided profile.`, "origin");
  const constitution = readNumber(finalAbilities, "constitution");
  ...
  if (readNumber(resources, "hitPointsMaximum") !== expectedHp || readNumber(resources, "hitPointsCurrent") !== expectedHp) pushError(issues, "dnd5e.guided.level-one-hp", `Level 1 ${classId} ${speciesId} must have ${expectedHp} Hit Points for this generated state.`, "resources.hitPointsMaximum");
  const expectedArmorClass = classId === "fighter" ? 17 : classId === "barbarian" ? 10 + abilityModifier(dexterity) + abilityModifier(constitution) : classId === "monk" ? 10 + abilityModifier(dexterity) + abilityModifier(wisdom) : 11 + abilityModifier(dexterity);
  if (readNumber(derived, "armorClass") !== expectedArmorClass) pushError(issues, "dnd5e.guided.armor-class", `Generated ${classId} Armor Class must be ${expectedArmorClass}.`, "derived.armorClass");
  ...
  if (readNumber(derived, "initiativeModifier") !== expectedInitiative) pushError(issues, "dnd5e.guided.initiative", `Generated Initiative must be ${expectedInitiative}.`, "derived.initiativeModifier");
  ...
  if (readNumber(derived, "passivePerception") !== expectedPassive) pushError(issues, "dnd5e.guided.passive-perception", `Generated Passive Perception must be ${expectedPassive}.`, "derived.passivePerception");
  if (classId === "fighter" && (readNumber(resources, "secondWindMaximum") !== 2 || readNumber(resources, "secondWindCurrent") !== 2)) pushError(issues, "dnd5e.fighter.second-wind", "Level 1 Fighter must retain two Second Wind uses.", "resources");
  if (classId === "barbarian" && (readNumber(resources, "rageMaximum") !== 2 || readNumber(resources, "rageCurrent") !== 2 || readNumber(resources, "rageDamageBonus") !== 2)) pushError(issues, "dnd5e.barbarian.rage", "Level 1 Barbarian must retain two Rage uses and +2 Rage Damage.", "resources");
  if (speciesId === "dwarf" && (readNumber(resources, "stonecunningMaximum") !== 2 || readNumber(resources, "stonecunningCurrent") !== 2)) pushError(issues, "dnd5e.dwarf.stonecunning", "Level 1 Dwarf must retain two Stonecunning uses at Proficiency Bonus +2.", "resources");
  if (speciesId === "orc" && (readNumber(resources, "adrenalineRushMaximum") !== 2 || readNumber(resources, "relentlessEnduranceMaximum") !== 1)) pushError(issues, "dnd5e.orc.resources", "Level 1 Orc must retain two Adrenaline Rush uses and one Relentless Endurance use.", "resources");
}
...
export const dnd5eSrd521Adapter: RulesSystemAdapter = {
  ...
  validateNativeState(state: NativeSystemState): RulesValidationResult {
    ...
    if (state.schemaVersion !== "dnd5e-character/0.1" && state.schemaVersion !== "dnd5e-character/0.2") pushError(issues, "adapter.schema-version", "Unsupported D&D native schema version.", "schemaVersion");
    ...
    if (!Array.isArray(rulesSourceIds) || !rulesSourceIds.includes(DND5E_SRD_5_2_1_SOURCE.id)) pushError(issues, "dnd5e.rules-source.missing", "Native state must retain the SRD 5.2.1 rules-source identifier.", "payload.rulesSourceIds");
