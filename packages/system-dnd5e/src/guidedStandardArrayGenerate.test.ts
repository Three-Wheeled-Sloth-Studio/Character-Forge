...
function payloadOf(character: ReturnType<typeof guidedStandardArrayGenerateDnd5eFirstSlice>): Dnd5eNativeCharacter { return character.nativeStates[0]!.payload as Dnd5eNativeCharacter; }
...
describe("guided SRD class and species generation", () => {
  it("catalogs the full SRD core and enables only faithfully modeled choices", () => {
    ...
    expect(GUIDED_DND5E_SPECIES_IDS).toEqual(["dragonborn", "dwarf", "elf", "gnome", "goliath", "halfling", "human", "orc", "tiefling"]);
    expect(DND5E_SRD_521_CLASS_OPTIONS.find((option) => option.id === "warlock")).toEqual(expect.objectContaining({ guidedSupported: true }));
    expect(DND5E_SRD_521_SPECIES_OPTIONS.every((option) => option.guidedSupported)).toBe(true);
  });

  it("builds and validates a Dwarf Soldier Barbarian with class/species-sensitive derived state", () => {
    const character = guidedStandardArrayGenerateDnd5eFirstSlice({ name: "Bryn Granite", classChoice: { selectedId: "barbarian", acceptableIds: ["barbarian", "fighter", "rogue"], selectionMode: "random" }, speciesChoice: { selectedId: "dwarf", acceptableIds: ["dwarf", "halfling", "human", "orc"], selectionMode: "direct" }, assignment, backgroundIncreases: { strength: 2, constitution: 1 } });
    const nativeState = character.nativeStates[0]!; const payload = payloadOf(character);
    expect(nativeState.schemaVersion).toBe("dnd5e-character/0.3"); expect(payload.class.classId).toBe("barbarian"); expect(payload.origin.speciesId).toBe("dwarf"); expect(payload.resources.hitPointsMaximum).toBe(15); expect(payload.resources.rageMaximum).toBe(2); expect(payload.resources.stonecunningMaximum).toBe(2); expect(payload.derived.armorClass).toBe(14); expect(payload.derived.initiativeModifier).toBe(2); expect(payload.derived.passivePerception).toBe(13);
  ...
  it("retains Dragonborn ancestry, damage type, and Breath Weapon uses", () => {
    const character = guidedStandardArrayGenerateDnd5eFirstSlice({ name: "Ember Scale", classChoice: { selectedId: "fighter", acceptableIds: ["fighter"], selectionMode: "direct" }, speciesChoice: { selectedId: "dragonborn", acceptableIds: ["dragonborn"], selectionMode: "direct" }, assignment, backgroundIncreases: { strength: 2, constitution: 1 } });
    const payload = payloadOf(character); expect(payload.origin.speciesAncestryId).toBe("red"); expect(payload.origin.speciesDamageType).toBe("fire"); expect(payload.resources.breathWeaponMaximum).toBe(2); expect(payload.resources.breathWeaponCurrent).toBe(2); expect(dnd5eSrd521Adapter.validateNativeState(character.nativeStates[0]!).valid).toBe(true);
  });

  it("retains Goliath Giant Ancestry, 35-foot speed, and ancestry uses", () => {
    const character = guidedStandardArrayGenerateDnd5eFirstSlice({ name: "Stone Walker", classChoice: { selectedId: "fighter", acceptableIds: ["fighter"], selectionMode: "direct" }, speciesChoice: { selectedId: "goliath", acceptableIds: ["goliath"], selectionMode: "direct" }, assignment, backgroundIncreases: { strength: 2, constitution: 1 } });
  ...
  it("builds all 108 enabled class/species combinations through one native-state boundary", () => {
    expect(GUIDED_DND5E_CLASS_IDS.length * GUIDED_DND5E_SPECIES_IDS.length).toBe(108);
    for (const classId of GUIDED_DND5E_CLASS_IDS) {
      for (const speciesId of GUIDED_DND5E_SPECIES_IDS) {
        ...
        expect(validation.valid, `${speciesId} ${classId}: ${validation.issues.map((issue) => issue.message).join(" | ")}`).toBe(true);
      }
    }
  });
