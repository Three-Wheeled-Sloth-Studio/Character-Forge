...
const WIZARD_LEVEL_ONE_SPELL_IDS = [
  ...
  "floating-disk", "fog-cloud", "grease", "hideous-laughter", "ice-knife", "identify", "illusory-script",
  "jump", "longstrider", "mage-armor", "magic-missile", "protection-from-evil-and-good", "ray-of-sickness",
...
export const DND5E_MAGIC_INITIATE_SPELL_LISTS: Record<Dnd5eMagicInitiateSpellListId, Dnd5eMagicInitiateSpellList> = {
  cleric: { id: "cleric", label: "Cleric", cantrips: CLERIC_CANTRIP_IDS.map(spellOption), levelOneSpells: CLERIC_LEVEL_ONE_SPELL_IDS.map(spellOption) },
  druid: { id: "druid", label: "Druid", cantrips: DRUID_CANTRIP_IDS.map(spellOption), levelOneSpells: DRUID_LEVEL_ONE_SPELL_IDS.map(spellOption) },
  wizard: { id: "wizard", label: "Wizard", cantrips: WIZARD_CANTRIP_IDS.map(spellOption), levelOneSpells: WIZARD_LEVEL_ONE_SPELL_IDS.map(spellOption) },
};
