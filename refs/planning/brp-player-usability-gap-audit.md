...
| Wealth boundaries | supported | Profession-specific creation now supports Destitute, Poor, Average, Affluent, and Wealthy where the implemented source profiles allow them. |
...
| Save/reopen | supported | The creator reconstructs state from the authoritative BRP primary native state, including player-core profession choices. |
...
| --- | --- | --- |
| Further profession/skill breadth | missing for v0.1 | The creator now has credible generic profession coverage, but some source professions still depend on skills or specialties not yet in the bounded catalog. Add only when they improve player coverage or finishing flow. |
...
The next highest-value gap was profession breadth. BRP explicitly treats its profession list as adaptable and defines a new profession by title, description/training/social position, wealth, and ten essential skills. The bounded player-core implementation therefore adds:

1. Athlete, with five fixed skills and five player-selected electives from the currently implemented source-backed subset;
2. Beggar, including Destitute/Poor wealth and its fixed ten-skill profile;
3. the remaining wealth levels required for source-shaped generic profession coverage;
4. Custom Profession with a title, description, wealth, and exactly ten essential professional skills retained losslessly in BRP native state; and
5. a player-core builder/validator layer that leaves the earlier Detective/Scholar and power-system probes unchanged.

The custom picker is intentionally limited to the current source-audited static skill catalog. Open specialty/language customization beyond Scholar, Ride/Martial Arts, and exhaustive profession coverage remain incremental breadth, not prerequisites for moving to equipment and finish-the-character work.
